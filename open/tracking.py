...
from servos import move_mg996r_servo
...
gradeservojos=90
gradeservosus=90
move_mg996r_servo(servo_pins[0], gradeservojos)
move_mg996r_servo(servo_pins[1], gradeservosus)




# Define the path to the folder for saving photos
output_folder = os.path.join(os.path.expanduser("~/Desktop"), "pozetest")
    ...
    if len(cnts) > 0:
        ...
        if cv2.contourArea(c) > 0:
            ...
            if radius > 10:
                ...
                abs_x = abs(center[0] - screen_center_x)
                abs_y = abs(center[1] - screen_center_y)
                ...
                if abs_x > abs_y:
                    ...
                    print(center[0] - screen_center_x)
                    if center[0] - screen_center_x < 0:
                        print("mutare dreapta")
                        move_mg996r_servo(servo_pins[0],gradeservojos+5)
                    else :
                        print("mutare dreapta")
                        move_mg996r_servo(servo_pins[0],gradeservojos-5)
                    
                else:
                    print("muatre servo sus")
                    print(center[1] - screen_center_y)
                    if center[1] - screen_center_y < 0:
                        
                        print("mutare sus")
                        move_mg996r_servo(servo_pins[1],gradeservosus+5)
                    else :
                        print("mutare jos")
                        move_mg996r_servo(servo_pins[1],gradeservosus-5)
